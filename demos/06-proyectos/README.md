# Demos de Proyectos

Casos relacionados con proyectos, tareas, responsables, etapas, tiempos y avances.

## Casos disponibles

- [Puesta en servicio de una subestación](demo-puesta-servicio-subestacion/README.md): proyecto con montaje, pruebas eléctricas y cierre documental.
- [Ciclo completo de una obra](demo-ciclo-completo-obra/README.md): cotización, diagnóstico, ejecución y entrega con cuatro tareas por etapa.
- [Cotización con partidas internas](02-cotizacion-partidas-internas/README.md): resumen comercial y desglose interno de costos.
- [Compras por proyecto](03-compras-por-proyecto/README.md): compras, recepción e imputación a una obra.
- [Inventario y trazabilidad en obra](04-inventario-y-trazabilidad-en-obra/README.md): transferencias entre bodega central y obra.
- [Gastos urgentes de terreno](05-gastos-de-terreno/README.md): comprobante, aprobación y reembolso.
- [Facturación del proyecto](06-facturacion-del-proyecto/README.md): emisión, cobro y seguimiento de facturas.
- [Rentabilidad del proyecto](07-rentabilidad-del-proyecto/README.md): ingresos, costos y margen de la obra.

## Orden recomendado

1. Puesta en servicio de una subestación.
2. Cotización con partidas internas.
3. Compras por proyecto.
4. Inventario y trazabilidad en obra.
5. Gastos urgentes de terreno.
6. Facturación del proyecto.
7. Rentabilidad del proyecto.
